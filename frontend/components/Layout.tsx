...
const Layout: React.FC<LayoutProps> = ({ children, userRole, onLogout }) => {
  ...
  const studentLinks = [
    ...
    { name: 'My Profile', path: '/student/my-profile', icon: UserCircle },
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <div className="h-10 w-10 rounded-full bg-slate-200 overflow-hidden border border-slate-300">
              <img
                src={
                  userRole === UserRole.STUDENT
                    ? (profile?.avatarUrl || `https://picsum.photos/id/64/100/100`)
                    : (companyData?.logoUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(companyData?.name || 'C')}&background=7c3aed&color=fff&size=100`)
                }
                alt="Avatar"
        ...
        <main className="flex-1 p-4 sm:p-6 lg:px-8 overflow-y-auto">
          {children}
        </main>
      </div>
    </div>
