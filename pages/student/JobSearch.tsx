...
const JobSearch = () => {
  ...
  const loadJobs = async () => {
    ...
    try {
        ...
        const data = await fetchJobs({ search: searchTerm }); 
  ...
  const handleApply = async (jobId: string) => {
    try {
        ...
        alert('Application failed. Make sure your profile is complete.');
    }
  };
    ...
    <div className="bg-white p-6 rounded-xl border border-slate-200 hover:border-brand-300 hover:shadow-md transition-all group relative">
        <div className="flex justify-between items-start mb-4">
            <div className="flex gap-4">
                <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center font-bold text-slate-400">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-50">
            <span className="text-xs text-slate-400">Posted {new Date(job.postedAt).toLocaleDateString()}</span>
            <button 
                onClick={onApply}
